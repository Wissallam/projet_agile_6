       ...
       <?php
        ...
        ?>  
                ...
                <form method="POST" action="{{route('projects.update',['project'=>$project->id])}}" enctype="multipart/form-data">
                    ...
                    @method('PUT')
                    <div class="mb-3">
                      <label class="label-material" for="name">Nom du project</label>
                      <input class="form-control" style="border-radius:20px;" id="name" name="name" value="{{old('name',$project->name)}}" type="text" aria-describedby="emailHelp">
                      @if ($errors->has('name'))
                      <span class="text-danger">{{ $errors->first('name') }}</span>
                  @endif
                    </div>
                    <div class="label-material" style="color:brown">Ancien chef de Projet:&nbsp;&nbsp;{{old('name',$resolveur->name)}}</div><br>
                    <div class="row">
                        <label class="label-material" for="resolveur">Nouveau chef de projet:</label>
